...
class Solver:
    ...
    def get_maze(self, file_name):
        """
        Load the maze from the given file.
        """
        with open(file_name, 'r') as file:
            self.board = [list(line.strip()) for line in file]  # Read the maze from the file into a 2D array
...
if __name__ == "__main__":
    ...
    output_directory = "solved_mazes"  # Directory to store the maze solutions
    if not os.path.exists(output_directory):  # Create the directory if it doesn't exist
        os.makedirs(output_directory)
    output_file_name = input("Enter a file name to save the solution (add .txt): ")  # Ask the user for a file name
    unsolved_solver = copy.deepcopy(solver)  # Create a copy of the maze
    solver.backtracking_solver()  # Solve the maze
    output_file_path = os.path.join(output_directory, output_file_name)  # Construct the path to the file
    with open(output_file_path, 'w') as output:  # Save the original maze and maze solution to the file
        output.write("Original Maze:\n")
        for row in unsolved_solver.board:
            output.write(' '.join(row) + '\n')
        output.write("\nSolution:\n")
        for row in solver.board:
            output.write(' '.join(row) + '\n')
    print("Solution saved to", output_file_path)